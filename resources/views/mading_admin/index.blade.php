        ...
        <div class="row mb-4">
            <div class="col-md-3">
                <div class="card border-0 shadow-sm rounded-4 bg-primary text-white overflow-hidden position-relative">
                    <div class="card-body p-4 position-relative" style="z-index: 2;">
                        <h4 class="fw-bold mb-1">{{ $announcements->total() }}</h4>
            ...
            <div class="col-md-3">
                <div class="card border-0 shadow-sm rounded-4 bg-success text-white overflow-hidden position-relative">
                    <div class="card-body p-4 position-relative" style="z-index: 2;">
                        @php $activeCount = \App\Models\Announcement::where('is_active', true)->count(); @endphp
            ...
            <div class="col-md-3">
                <div class="card border-0 shadow-sm rounded-4 bg-warning text-dark overflow-hidden position-relative">
                    <div class="card-body p-4 position-relative" style="z-index: 2;">
                        @php $runningCount = \App\Models\Announcement::where('type', 'running_text')->count(); @endphp
                        <h4 class="fw-bold mb-1">{{ $runningCount }}</h4>
            ...
            <div class="col-md-3">
                <div class="card border-0 shadow-sm rounded-4 bg-info text-white overflow-hidden position-relative">
                    <div class="card-body p-4 position-relative" style="z-index: 2;">
                        @php $infoCount = \App\Models\Announcement::whereIn('type', ['news', 'poster'])->count(); @endphp
                        <h4 class="fw-bold mb-1">{{ $infoCount }}</h4>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-body p-3">
                        <form action="{{ route('mading-admin.index') }}" method="GET" class="row g-2 align-items-center">
                            <div class="col-md-5">
                                <div class="input-group input-group-sm">
                                    <span class="input-group-text bg-light border-0"><i class="bi bi-search"></i></span>
                                    <input type="text" name="search" class="form-control border-0 bg-light" placeholder="Cari judul konten..." value="{{ request('search') }}">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <select name="unit_id" class="form-select form-select-sm border-0 bg-light">
                            ...
                            <div class="col-md-3 d-flex gap-2">
                                <button type="submit" class="btn btn-primary btn-sm rounded-pill px-3 flex-grow-1">Filter</button>
                                <a href="{{ route('mading-admin.index') }}" class="btn btn-light btn-sm rounded-pill px-3">Reset</a>
                            </div>
                        </form>
                    </div>
                </div>

                <form id="bulkActionForm" action="{{ route('mading-admin.bulk-destroy') }}" method="POST">
                    @csrf
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                        <div class="card-header bg-white py-3 border-bottom border-light">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center gap-3">
                                    <h5 class="card-title fw-bold mb-0">Daftar Konten Mading</h5>
                                    <div id="bulkActions" class="d-none animate__animated animate__fadeIn">
                                        <div class="btn-group shadow-sm rounded-pill overflow-hidden border">
                                            <button type="submit" class="btn btn-white btn-sm px-3" onclick="setBulkAction(event, '{{ route('mading-admin.bulk-update') }}', 'active')">
                                                <i class="bi bi-check-circle text-success me-1"></i>Aktifkan
                                            </button>
                                            <button type="submit" class="btn btn-white btn-sm px-3" onclick="setBulkAction(event, '{{ route('mading-admin.bulk-update') }}', 'inactive')">
                                                <i class="bi bi-dash-circle text-secondary me-1"></i>Non-aktifkan
                                            </button>
                                            <button type="submit" class="btn btn-white btn-sm px-3" onclick="setBulkAction(event, '{{ route('mading-admin.bulk-destroy') }}')">
                                                <i class="bi bi-trash3 text-danger me-1"></i>Hapus (<span id="selectedCount">0</span>)
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                <button type="button" class="btn btn-light btn-sm text-primary rounded-pill px-3" onclick="window.location.reload()">
                        ...
                        <div class="card-body p-0">
                            ...
                            @endif
                            @if(session('error'))
                                <div class="alert alert-danger border-0 rounded-0 mb-0 shadow-sm" role="alert">
                                        ...
                                        <tr>
                                            ...
                                            <td class="pe-4 text-end">
                                                <div class="btn-group shadow-sm rounded-3 overflow-hidden">
                                                    ...
                                                    <button type="button" class="btn btn-white btn-sm px-3" title="Hapus" onclick="confirmDelete('{{ route('mading-admin.destroy', $item->id) }}')">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateBulkActions() {
        ...
        if (checkedCount > 0) {
            ...
            const counters = document.querySelectorAll('#selectedCount');
            counters.forEach(c => c.textContent = checkedCount);
        } else {
            bulkActions.classList.add('d-none');
        }
    }

    window.setBulkAction = function(e, action, status = null) {
        ...
        if (status) {
            ...
            if (!confirm('Hapus semua konten yang dipilih?')) {
                e.preventDefault();
                return false;
            }
        }
